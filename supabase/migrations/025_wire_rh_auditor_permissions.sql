-- Wires up what rh and auditor actually do, per the spec's authorization matrix (section 3):
--   Auditor: read-only everywhere. Needs nothing here — can_access_store already grants read
--   access to any store_membership row regardless of role, and can_manage_store (the gate on
--   every write path) already excludes any role not explicitly listed. Adding 'auditor' as a
--   valid membership role, with no matching write grant anywhere, already gives exactly that.
--
--   RH/DP: "Editar rascunho: Sim", "Aprovar escala: Sim", "Publicar: Configurável" (treated as
--   allowed here — there is no per-organization toggle to make it configurable), all matching
--   what can_manage_store already gates (save_canonical_entry, submit/decide approval, publish).
--   "Alterar regras: Sim" is gated separately from can_manage_store in create_rule_set_revision
--   (manager is deliberately excluded from that one per "Alterar regras: Gerente Não"), so it
...
begin
  ...
  if not exists (
    ...
    where m.user_id = auth.uid() and m.role in ('owner', 'rh') and s.organization_id = current_set.organization_id
  ) then raise exception 'Only an organization owner or RH/DP can create a new rule set version'; end if;
