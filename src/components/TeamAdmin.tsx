...
export function TeamAdmin({ storeId }: { storeId: string }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await inviteStoreMember({ storeId, email: String(data.get('email')), role: String(data.get('role')) as 'manager' | 'supervisor' | 'employee' | 'rh' | 'auditor', sectorIds: data.getAll('sectors').map(String), canEditSector: data.get('canEdit') === 'on' })
  ...
  return <section className="team-admin" id="team"><div className="audit-title"><div><p className="eyebrow">ADMINISTRAÇÃO</p><h2>Equipe e permissões</h2><p>Convide gestores, RH, fiscais, auditores e colaboradores e limite o acesso por setor.</p></div><span className="audit-rule"><ShieldCheck size={13} />Acesso por loja</span></div><div className="team-grid"><div className="team-list"><h3>Membros da loja</h3>{team.map((member) => <div className="team-member" key={member.user_id}><span>{member.full_name.slice(0, 2).toUpperCase()}</span><div><strong>{member.full_name}</strong><small>{roleLabel[member.role] ?? member.role}{member.can_edit_sector ? ' · edita setores' : ''}</small></div></div>)}{!team.length && <p className="empty">Nenhum membro encontrado.</p>}</div><form className="invite-form" onSubmit={invite}><h3><Plus size={16} />Convidar pessoa</h3><label>E-mail<input required name="email" type="email" placeholder="nome@empresa.com" /></label><label>Papel<select name="role" defaultValue="employee"><option value="employee">Colaborador · consulta própria</option><option value="supervisor">Fiscal/encarregado · escopo setorial</option><option value="auditor">Auditor · somente leitura</option><option value="rh">RH/DP · regras, aprovação e colaboradores</option><option value="manager">Gerente · loja inteira</option></select></label><fieldset><legend>Setores autorizados</legend>{sectors.map((sector) => <label key={sector.id} className="check"><input name="sectors" value={sector.id} type="checkbox" />{sector.name}</label>)}</fieldset><label className="check"><input name="canEdit" type="checkbox" />Pode editar os setores selecionados</label>{message && <p className="invite-message">{message}</p>}<button className="solid" disabled={loading} type="submit"><Mail size={16} />{loading ? 'Enviando...' : 'Enviar convite'}</button></form></div></section>
}
